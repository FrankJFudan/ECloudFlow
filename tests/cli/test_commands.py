...
def test_config_show_accepts_hydra_override():
    result = CliRunner().invoke(app, ["config", "show", "--override", "sample.profile=fast"])
...
def test_data_prepare_writes_resolved_manifest(tmp_path):
    result = CliRunner().invoke(app, ["data", "prepare", "--output-dir", str(tmp_path), "--dataset", "pdbbind"])
    assert result.exit_code == 0, result.stdout
...
def test_train_dry_run_writes_launch_config(tmp_path):
    result = CliRunner().invoke(app, ["train", "--dry-run", "--output-dir", str(tmp_path), "--max-steps", "2"])
    assert result.exit_code == 0, result.stdout
